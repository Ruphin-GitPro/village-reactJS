    ...
    <div style={{ paddingTop: 64 }}>
      <section style={{ padding: '5rem 1.5rem 3rem', textAlign: 'center', background: 'linear-gradient(180deg,#090b1a,#06081c)' }}>
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          <span style={{ color: '#d2691e', fontSize: '0.8rem', letterSpacing: 2, textTransform: 'uppercase' }}>En savoir plus</span>
          <h1 style={{ fontFamily: 'Playfair Display', fontSize: 'clamp(2.2rem,4vw,3.5rem)', color: '#fff', margin: '0.5rem 0 1rem' }}>
            Mieux comprendre Ambohipisaka
          </h1>
          <p style={{ color: '#9aa3c4', maxWidth: 600, margin: '0 auto', lineHeight: 1.75 }}>
            Toutes les informations pour mieux comprendre la vie à Ambohipisaka.
          </p>
        </motion.div>
      </section>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '3rem 1.5rem 5rem' }}>
        {[
          { title: 'Les quartiers', content: 'Amboniandrefana et Andafiavaratra sont des quartiers dynamiques où les familles cultivent leurs champs et partagent des moments de solidarité.' },
          { title: 'Vie quotidienne', content: 'Le village vit au rythme des saisons : culture du riz, élevage, préparation des repas et fêtes de village.' },
          { title: 'Images de la vie locale', content: 'Découvrez quelques aspects de la vie quotidienne à Ambohipisaka.' },
        ].map((s, i) => (
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} style={cardStyle}>
          <h2 style={{ fontFamily: 'Playfair Display', fontSize: '1.5rem', color: '#fff', marginBottom: '0.5rem' }}>Carte du village</h2>
          <p style={{ color: '#9aa3c4', marginBottom: '1.2rem', lineHeight: 1.7 }}>
            Voici une carte pour localiser le village dans la région Bongolava.
          </p>
